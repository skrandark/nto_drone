...
def coords():
    ...
    for i in range(len(fire) - 1):
        if abs(fire[i][0] - fire[i + 1][0]) > 0.4 or abs(fire[i][1] - fire[i + 1][1]) > 0.4 or i == len(fire) - 2:
...
def mission_routine():
    ...
    mission_active = True
    land_now = False
    set_effect(r=255, g=165, b=0)  # Зеленый свет при старте
...
def mission1(data, client_socket):
    for x, y in data:
        if not navigate_wait(x=x, y=y, z=1.5, frame_id='aruco_map'):
            return
        rospy.sleep(2)
        set_effect(effect='blink', r=0, g=0, b=255)
        if not navigate_wait(x=x, y=y, z=1, frame_id='aruco_map'):
            return
        rospy.sleep(2)
        if not navigate_wait(x=x, y=y, z=1.5, frame_id='aruco_map'):
            return
        set_effect(r=255, g=165, b=0)
    if not navigate_wait(x=x, y=y, z=1.5, frame_id='aruco_4'):
        return
    send_variable('2_mission_end', client_socket)
...
def start_mission():
    ...
    current_mission_thread = threading.Thread(target=mission_routine)
